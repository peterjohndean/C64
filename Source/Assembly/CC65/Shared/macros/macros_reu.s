...
; TOOLS   : CC65, VICE emulator, physical C64U
; ============================================================
; PURPOSE
; -------
...
; ============================================================
.macro REU_QUICK_DETECT label
	.if .paramcount <> 1
		.error  "Too few parameters for macro REU_QUICK_DETECT"
	.endif
...
.macro REU_FROM_C64 rAddress, cAddress, tLength, tCommand
	.if .paramcount < 3
		.error  "Too few parameters for macro REU_FROM_C64"
	.endif
    ...
    .ifblank tCommand
        lda #REU_CMD_C64_TO_REU	; Default
    .else
        lda #tCommand			; Use provided command
    .endif
...
.macro REU_TO_C64 rAddress, cAddress, tLength, tCommand
    .if .paramcount < 3
		.error  "Too few parameters for macro REU_TO_C64"
	.endif
    ...
    .ifblank tCommand
        lda #REU_CMD_REU_TO_C64	; Default
    .else
        lda #tCommand			; Use provided command
    .endif
...
; ============================================================
.macro REU_SWAP rAddress, cAddress, tLength, tCommand
	.if .paramcount < 3
		.error  "Too few parameters for macro REU_SWAP"
	.endif
...
.macro REU_FROM_C64_B rBank, rOffset, cAddress, tLength, tCommand
	.if .paramcount < 4
		.error  "Too few parameters for macro REU_FROM_C64_B"
	.endif
...
.macro REU_TO_C64_B rBank, rOffset, cAddress, tLength, tCommand
	.if .paramcount < 4
		.error  "Too few parameters for macro REU_TO_C64_B"
	.endif
...
; ============================================================
.macro REU_SET_LENGTH_MACRO tLength
	.if .paramcount <> 1
		.error  "Too few parameters for macro REU_SET_LENGTH_MACRO"
	.endif
	
    ; --- Set transfer length ---
    ; 16-bit value specifies number of bytes to transfer
    ; valid range: 1-65536 (0 is interpreted as 65536)
    lda #<tLength	; load low  byte of transfer length
    sta REU_LEN_LO	; $DF07 / 57095 - transfer length low
    lda #>tLength	; load high byte of transfer length
...
; ============================================================
.macro REU_SET_C64_ADDRESS_MACRO cAddress
	.if .paramcount <> 1
		.error  "Too few parameters for macro REU_SET_C64_ADDRESS_MACRO"
	.endif
...
;               (CC65 carrot ^ operator extracts byte 2)
; Notes   : Does NOT trigger a transfer. You must still write
;           to REU_COMMAND ($DF01) to initiate DMA.
;
;           Uses the CC65 ^ operator to extract the bank byte:
...
; ============================================================
.macro REU_SET_REU_ADDRESS_MACRO rAddress
	.if .paramcount <> 1
		.error  "Too few parameters for macro REU_SET_REU_ADDRESS_MACRO"
	.endif
    ...
    ; note: use ^ operator in CC65 for byte 2 of 24-bit value
.endmacro
