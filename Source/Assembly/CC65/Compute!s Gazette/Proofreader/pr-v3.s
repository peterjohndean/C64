...
;
; CC65
; cl65 -g -t c64 -C pr-v3.cfg -Ln pr-v3-vice.txt -m pr-v3-map.txt -l pr-v3-list.txt pr-v3.s -o pr-v3.bin
;
	.segment "LOADADDR"
    .word $0801
...
Preserve:
		.res $1f, $00				; Save/Restore blocks expects size $1f (32 bytes)
...
;
; -------------------------------------------------------------------------
; Shifting to the $0901 memory area via the linker TEXTTAB segment
; -------------------------------------------------------------------------
    	.segment "TEXTTAB"
...
; Replace the ACME !warn size check with ca65 .assert
    	.assert (BLOCK_END - BLOCK_BEGIN) <= 192, warning, "Wedge won't fit into Tape Buffer: > 192 bytes max."
